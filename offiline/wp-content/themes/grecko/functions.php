		...
		foreach ($theme_fonts as $font=>$v) {
			if (isset($fonts[$font])) {
				...
				$css = !empty($fonts[$font]['css']) 
					? $fonts[$font]['css'] 
					: themerex_get_protocol() . '://fonts.googleapis.com/css?family='.(!empty($fonts[$font]['link']) ? $fonts[$font]['link'] : str_replace(' ', '+', $font_name).':100,100italic,300,300italic,400,400italic,700,700italic').'&subset=latin,latin-ext,cyrillic,cyrillic-ext';
...
*/
				array(
					...
					'source'	=> themerex_get_file_dir('/plugins/new-royalslider.zip'),
					'required' 	=> false
				),
				array(
...
if ( !function_exists( 'themerex_filter_where' ) ) {
	function themerex_filter_where($where = '') { 
		...
		if (themerex_strpos($where, 'post_status')===false && (!isset($_REQUEST['preview']) || $_REQUEST['preview']!='true') && (!isset($_REQUEST['vc_editable']) || $_REQUEST['vc_editable']!='true')) {
			...
			$where .= " AND ".(!empty($prv) ? '(' : '')."{$wpdb->posts}.post_status='publish'".(!empty($prv) ? $prv : '').(!empty($prv) ? ')' : '');
		}
		return $where;
	}  
}
...
if ( !function_exists( 'themerex_callback_send_contact_form' ) ) {
	function themerex_callback_send_contact_form() {
		...
		if (!($contact_email = get_theme_option('contact_email')) && !($contact_email = get_theme_option('admin_email'))) 
			...
			if ($type=='contact') {
				$user_name	= getShortString($post_data['username'],100);
				$user_email	= getShortString($post_data['email'], 	100);
				$user_subj	= getShortString($post_data['subject'], 100);
				$user_msg	= getShortString($post_data['message'], max(100, get_theme_option('message_maxlength_contacts')));
...
if ( !function_exists( 'themerex_callback_registration_user' ) ) {
	function themerex_callback_registration_user() {
		...
		if ( is_wp_error($id) ) {
		...
		} else if (($notify = get_theme_option('notify_about_new_registration'))!='no' && (($contact_email = get_theme_option('contact_email')) || ($contact_email = get_theme_option('admin_email')))) {
			$mail = get_theme_option('mail_function');
			...
			if (in_array($notify, array('both', 'user', 'yes'))) {
				...
				$msg = "\n".__('Your registration data:', 'themerex')
					."\n".__('Name:', 'themerex').' '.$user_name
					."\n".__('E-mail:', 'themerex').' '.$user_email
					."\n".__('Password:', 'themerex').' '.$user_pwd
					."\n\n............ " . get_bloginfo('site_name') . " (<a href=\"" . home_url() . "\">" . home_url() . "</a>) ............";
				$head = "Content-Type: text/plain; charset=\"utf-8\"\n"
					. "X-Mailer: PHP/" . phpversion() . "\n"
