...
function sc_audio($atts, $content = null) {
	...
	$output = '<audio' . ($id ? ' id="' . $id . '"' : '') . (!empty($class) ? ' class="'.$class.'"' : '')
 . ' src="' . $src . '" class="sc_audio" ' . (sc_param_is_on($controls) ? ' controls="controls"' : '') . (sc_param_is_on($autoplay) && is_single() ? ' autoplay="autoplay"' : '') . ' width="' . $width . '" height="' . $height .'"'.($s!='' ? ' style="'.$s.'"' : '').'></audio>';
	return apply_filters('themerex_shortcode_output', $output, 'trx_audio', $atts, $content);
}
// ---------------------------------- [/trx_audio] ---------------------------------------
...
function sc_banner($atts, $content=null){	
	...
	$output =  empty($src) ? '' : ('<a href="' . (empty($link) ? '#' : $link) . '"' . ($id ? ' id="' . $id . '"' : '')
		...
		. '</a>');
	return apply_filters('themerex_shortcode_output', $output, 'trx_banner', $atts, $content);
}
...
function sc_br($atts, $content = null) {
	...
	$output = '<br' . (in_array($clear, array('left', 'right', 'both')) ? ' style="clear:' . $clear . '"' : '') . ' />';
...
function sc_blogger($atts, $content=null){	
	...
	if (!in_array($style, array('regular','date','image_large','image_medium','image_small','image_tiny','accordion_1','accordion_2','list','excerpt','related')) 
...
function sc_button($atts, $content=null){	
	...
	$output = '<div class="sc_button sc_button_style_' . $style 
			. ' sc_button_size_' . $size 
			. ($align && $align!='none' ? ' align'.$align : '') 
			. (!empty($class) ? ' '.$class : '')
			. ' ' . $type . 'Button' 
			. (sc_param_is_on($fullsize) ? ' fullSize' : '') 
			. ' ' . $style 
			. ' ' . $size 
			. ($icon!='' ? '  ico' : '') 
			. '"' 
...
function sc_chat($atts, $content=null){	
	...
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_chat' . (!empty($class) ? ' '.$class : '') . '"' . ($s ? ' style="'.$s.'"' : '') . '>'
		. $content
...
function sc_columns($atts, $content=null){	
	...
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') 
				. ' class="columnsWrap sc_columns sc_columns_count_' . $count . (!empty($class) ? ' '.$class : '') . '"'
				. ($s!='' ? ' style="'.$s.'"' : '').'>' 
				. do_shortcode($content)
				. '</div>';
...
function sc_contact_form($atts, $content = null) {
	...
	$THEMEREX_sc_contact_form_counter = 0;
	$content = do_shortcode($content);
	$output = '<div ' . ($id ? ' id="' . $id . '"' : '') . 'class="sc_contact_form sc_contact_form_'.($content ? 'custom' : 'contact').(!empty($class) ? ' '.$class : '').'"'.($s!='' ? ' style="'.$s.'"' : '') .'>'
...
function sc_countdown($atts, $content = null) {
	...
	if (date('Y-m-d H:i:s') < $date.' '.($time ? $time : '00:00:00')) {
		...
		$s = getStyleString($top, $right, $bottom, $left, $width, $height);
		$output = '<div' . ($id ? ' id="' . $id . '"' : '')
					. ' class="sc_countdown_wrapper' . ($align && $align!='none' ? ' align' . $align : '') . (!empty($class) ? ' '.$class : '') . '"'
					. ($s ? ' style="'.$s.'"' : '')
					. '>'
						. '<div class="sc_countdown" data-date="'.$date.'" data-time="'.$time.'"></div>'
					. '</div>';
	}
...
function sc_dropcaps($atts, $content=null){
	...
	$content = do_shortcode($content);
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_dropcaps sc_dropcaps_style_' . $style . (!empty($class) ? ' '.$class : '') . '">' 
...
function sc_emailer($atts, $content = null) {
	...
	$output = '<div' . ($id ? ' id="' . $id . '"' : '')
				. ' class="sc_emailer inputSubmitAnimation' 
					. ($align && $align!='none' ? ' sc_align' . $align : '') 
					. (sc_param_is_on($open) ? ' sFocus rad4 opened' : ' radCircle') 
					. (!empty($class) ? ' '.$class : '') 
					. '"' 
				. ($s ? ' style="'.$s.'"' : '') 
				. '>'
...
function sc_gap($atts, $content = null) {
	if (in_shortcode_blogger()) return '';
	$output = sc_gap_start() . do_shortcode($content) . sc_gap_end();
...
function sc_highlight($atts, $content=null){	
	...
	$s = ($color != '' ? 'color:' . $color . ';' : '')
		...
		.($style != '' ? $style : '');
	$output = '<span' . ($id ? ' id="' . $id . '"' : '') 
				. ' class="sc_highlight'.($type>0 ? ' sc_highlight_style_'.$type : ''). (!empty($class) ? ' '.$class : '').'"'
				. ($s!='' ? ' style="'.$s.'"' : '')
				. '>' 
					. do_shortcode($content) 
				. '</span>';
...
function sc_image($atts, $content=null){	
	...
	$output = empty($src) ? '' : ('<figure' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_image ' . ($align && $align!='none' ? ' sc_image_align_' . $align : '') . (!empty($shape) ? ' sc_image_shape_'.$shape : '') . (!empty($class) ? ' '.$class : '') . '"'.($s!='' ? ' style="'.$s.'"' : '').'>'
...
function sc_infobox($atts, $content=null){	
	...
	$s = getStyleString($top, $right, $bottom, $left);
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') 
				. ' class="sc_infobox sc_infobox_style_' . $style . (sc_param_is_on($closeable) ? ' sc_infobox_closeable' : '') . (!empty($class) ? ' '.$class : '') . '"'
				. ($s!='' ? ' style="'.$s.'"' : '')
				. '>'
					. do_shortcode($content) 
				. '</div>';
...
function sc_line($atts, $content=null){	
	...
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') 
				. ' class="sc_line' . ($style != '' ? ' sc_line_style_' . $style : '') . (!empty($class) ? ' '.$class : '') . '"'
				. ($s!='' ? ' style="'.$s.'"' : '')
				. '>'
				. '</div>';
...
function sc_list($atts, $content=null){	
	...
	$output = '<' . ($style=='ol' ? 'ol' : 'ul') . ($id ? ' id="' . $id . '"' : '') 
				. ' class="sc_list sc_list_style_' . $style . (!empty($class) ? ' '.$class : '') . '"' 
				. ($s!='' ? ' style="'.$s.'"' : '') 
				. '>'
					. do_shortcode($content) 
				. '</' .($style=='ol' ? 'ol' : 'ul') . '>';
...
function sc_parallax($atts, $content=null){	
	...
	$s = getStyleString($top, '!'.$right, $bottom, '!'.$left, $width, $height)
		...
		;
	$output = (sc_param_is_on($gap) ? sc_gap_start() : '')
		...
		. ($video!='' 
			? '<div class="sc_video_bg_wrapper">'
				. '<video class="sc_video_bg" width="'.$video_width.'" height="'.$video_height.'" data-width="'.$video_width.'" data-height="'.$video_height.'" preload="metadata" autoplay="autoplay" loop="loop" src="'.esc_attr($video).'" data-ratio="'.esc_attr($video_ratio).'" data-frame="no">'
...
function sc_popup($atts, $content=null){	
	...
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') 
				. ' class="sc_popup sc_popup_light mfp-with-anim mfp-hide' . ($class ? ' '.$class : '') . '"'
				. ($s!='' ? ' style="'.$s.'"' : '')
				. '>' 
					. do_shortcode($content) 
				. '</div>';
...
function sc_price_table($atts, $content=null){	
	...
	$count = max(1, $count);
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') 
				. ' class="sc_pricing_table columns_' . $count . ($align && $align!='none' ? ' align'.themerex_strtoproper($align) : '') . (!empty($class) ? ' '.$class : '') . '"' 
				. ($s!='' ? ' style="'.$s.'"' : '') 
				. '>'
					. do_shortcode($content)
			. '</div>';
	return apply_filters('themerex_shortcode_output', $output, 'trx_price_table', $atts, $content);
}
...
function sc_price_item($atts, $content=null) {
	...
	$output = '<div class="sc_pricing_columns sc_pricing_column_'.$THEMEREX_sc_price_table_counter. (!empty($class) ? ' '.$class : '').'">'
					. '<ul'.(sc_param_is_on($animation) ? ' class="columnsAnimate"' : '') . ($id ? ' id="' . $id . '"' : '') . '>'
						. do_shortcode($content) 
					. '</ul>'
				. '</div>';
	return apply_filters('themerex_shortcode_output', $output, 'trx_price_item', $atts, $content);
}
...
function sc_price_data($atts, $content=null) {
	...
	if ($type=='price' && $money!='') {
		...
		$content = do_shortcode($content);
	$output = '<li' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_pricing_data' . ($type!='' ? ' sc_pricing_'.$type : '') . (!empty($class) ? ' '.$class : '') . '">' 
				. $content 
			. '</li>';
	return apply_filters('themerex_shortcode_output', $output, 'trx_price_data', $atts, $content);
}
...
function sc_quote($atts, $content=null){	
	...
	$content = do_shortcode($content);
	if (themerex_substr($content, 0, 2)!='<p') $content = '<p>' . $content . '</p>';
	$output = '<blockquote' . ($id ? ' id="' . $id . '"' : '') . $cite_param . ' class="sc_quote'. (!empty($class) ? ' '.$class : '').'"' . ($s!='' ? ' style="'.$s.'"' : '') . '>'
				. $content
...
function sc_skills($atts, $content=null){	
	...
	$content = do_shortcode($content);
	$output = ($type!='arc' && $title!='' ? '<h2>'.$title.'</h2>' : '')
			. '<div' 
				. ($id ? ' id="' . $id . '"' : '') 
...
function sc_slider_item($atts, $content=null) {
	...
	$output = empty($src) ? '' : ('<li' . ($id ? ' id="' . $id . '"' : '').' class="' . $THEMEREX_sc_slider_engine.'-slide' . (!empty($class) ? ' '.$class : '') . '"'
...
function sc_table($atts, $content=null){	
	...
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') 
				. ' class="sc_table sc_table_style_' . max(1, min(4, $style)) 
					. ' sc_table_size_' . $size . (!empty($align) ? ' sc_table_align_'.$align : '') 
					. (!empty($class) ? ' '.$class : '') 
					. '"'
				. ($s!='' ? ' style="'.$s.'"' : '') 
				. '>' 
...
function sc_tabs($atts, $content = null) {
	...
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_tabs sc_tabs_style_'.$style. (!empty($class) ? ' '.$class : '') . '"'.($s!='' ? ' style="'.$s.'"' : '') .' data-active="' . ($initial-1) . '">'
	...
	$output .= $titles_output
		. '</ul>' 
		. $content
		.'</div>';
	return apply_filters('themerex_shortcode_output', $output, 'trx_tabs', $atts, $content);
}
...
function sc_tab($atts, $content = null) {
	...
	$output = '<div id="' . $id . '"'
					. ' class="sc_tabs_content' . ($THEMEREX_sc_tab_counter % 2 == 1 ? ' odd' : ' even') 
					. ($THEMEREX_sc_tab_counter == 1 ? ' first' : '') 
					. (!empty($class) ? ' '.$class : '') 
					. '"'
				. '>' 
					. (sc_param_is_on($THEMEREX_sc_tab_scroll) 
						? '<div id="'.$id.'_scroll" class="sc_scroll sc_scroll_vertical" style="height:'.($THEMEREX_sc_tab_height != '' ? $THEMEREX_sc_tab_height : '230px').';">'
							. '<div class="sc_scroll_wrapper swiper-wrapper">'
							.	 '<div class="sc_scroll_slide swiper-slide">' 
						: '')
							. do_shortcode($content) 
					. (sc_param_is_on($THEMEREX_sc_tab_scroll) 
						? '</div></div><div id="'.$id.'_scroll_bar" class="sc_scroll_bar sc_scroll_bar_vertical '.$id.'_scroll_bar"></div></div>' 
						: '')
			. '</div>';
...
function sc_team($atts, $content=null){	
	...
	$THEMEREX_sc_team_counter = 0;
	$content = do_shortcode($content);
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_team'.(!empty($class) ? ' '.$class : '').'"'.($s!='' ? ' style="'.$s.'"' : '') .'>'
...
function sc_team_item($atts, $content=null) {
	...
	if (!empty($name) && !empty($position)) {
		$output = '<div class="columns1_'.$THEMEREX_sc_team_count . (!empty($class) ? ' '.$class : '').'">'
...
function sc_testimonials($atts, $content=null){	
	...
	$content = do_shortcode($content);
	$output = ($title && $style>1 ? '<h2 class="sc_testimonials_title">'.$title.'</h2>' : '')
...
function sc_testimonials_item($atts, $content=null){	
	...
	$output = '<li' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_testimonials_item swiper-slide'. (!empty($class) ? ' '.$class : '').'" style="'.(!empty($THEMEREX_sc_testimonials_width) ? 'width:' . $THEMEREX_sc_testimonials_width . ';' : '').(!empty($THEMEREX_sc_testimonials_height) ? 'height:' . $THEMEREX_sc_testimonials_height . ';' : '').'">'
...
function sc_tooltip($atts, $content=null){	
	...
	$output = '<span' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_tooltip_parent'. (!empty($class) ? ' '.$class : '').'">' 
					. do_shortcode($content) 
...
function sc_zoom($atts, $content=null){	
	...
	$output = empty($src) ? '' : ((!sc_param_is_off($border) ? '<div class="sc_border sc_border_'.$border.'">' : '')
