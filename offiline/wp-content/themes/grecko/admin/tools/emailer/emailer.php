	...
	function build_page() {
		...
		if ( isset($_POST['emailer_subject']) ) {
			do {
				...
				// Get post data
				$subject = getValueGP('emailer_subject');
				if (empty($subject)) {
					$this->error = __('Subject can not be empty! Operation canceled!', 'themerex');
					break;
				}
				$message = getValueGP('emailer_message');
				if (empty($message)) {
				...
				$group   = getValueGP('emailer_group');
				$subscribers = getValueGP('emailer_subscribers');
				...
				$sender_name = getValueGP('emailer_sender_name', get_bloginfo('name'));
				$sender_email = getValueGP('emailer_sender_email');
