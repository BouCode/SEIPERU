...
class TRX_Importer {
	...
	var $options = array(
		...
		'page_on_front'		=> 'Home page',					// Homepage title
			...
			<?php if (empty($this->success) && $this->options['enable_importer']) { ?>
				...
				<form id="trx_importer_form" action="#" method="post">
					...
					<div class="trx_buttons">
						...
						<input type="submit" value="<?php printf(__('Continue Import (from ID=%s)', 'themerex'), $this->import_last_id); ?>" onClick="this.form.last_id.value='<?php echo $this->import_last_id; ?>'">
						<?php } ?>
					</div>
			...
			<?php if (empty($this->success) && $this->options['enable_exporter']) { ?>
				...
				<form id="trx_exporter_form" action="#" method="post">
					...
					<div class="trx_buttons">
						...
						<textarea rows="10" cols="80"><?php echo $this->export_options; ?></textarea>
						<?php if (false) { ?>
						<h4><?php _e('Post meta', 'themerex'); ?></h4>
						<textarea rows="10" cols="80"><?php echo $this->export_postmeta; ?></textarea>
						<?php } ?>
						<h4><?php _e('Widgets', 'themerex'); ?></h4>
						<textarea rows="10" cols="80"><?php echo $this->export_widgets; ?></textarea>
						<h4><?php _e('Royal Sliders', 'themerex'); ?></h4>
						<textarea rows="10" cols="80"><?php echo $this->export_sliders; ?></textarea>
						<?php } else { ?>
...
/*
		// Export post and page options
		$rows = $wpdb->get_results( "SELECT post_id, meta_key, meta_value FROM $wpdb->postmeta WHERE meta_key='post_custom_options'" );
		$options = array();
		if (count($rows) > 0) {
			foreach ($rows as $row) {
				$options[$row->post_id] = array('key'=>$row->meta_key, 'value'=>unserialize($row->meta_value));
			}
		}
		$this->export_postmeta = base64_encode(serialize($options));
*/
		// Export widgets
	...
	function checkRequiredPlugins() {
		...
		if (!royalslider_exists())
			$not_installed .= '<br>Royal Slider plugin';
		//if (!revslider_exists())
	...
	function clear_tables() {
		...
		if ($this->options['overwrite_content']) {
			...
			if ($this->options['import_sliders']) {
				$res = $wpdb->query("TRUNCATE TABLE {$wpdb->prefix}new_royalsliders");
				if ( is_wp_error( $res ) ) echo __( 'Failed truncate table "new_royalsliders".', 'themerex' ) . ' ' . $res->get_error_message() . '<br />';
			}
		}
	}
	...
	function import_sliders() {
		...
		if (!empty($this->options['file_with_royal_sliders'])) {
			echo '<br><b>'.__('Import Royal sliders ...', 'themerex').'</b><br>'; flush();
			global $wpdb;
