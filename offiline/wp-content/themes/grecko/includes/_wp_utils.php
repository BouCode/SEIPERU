...
if (!function_exists('getTermsByTaxonomy')) {
	function getTermsByTaxonomy($tax_types = array('post_format')) {
		global $wpdb, $wp_query;
		$taxes = $wpdb->get_results("SELECT DISTINCT terms.*, tax.taxonomy, tax.parent, tax.count"
				. " FROM " . esc_sql($wpdb->terms) . " AS terms"
				. " LEFT JOIN " . esc_sql($wpdb->term_taxonomy) . " AS tax ON tax.term_taxonomy_id=terms.term_id"
...
if (!function_exists('showSinglePageNav')) {
	function showSinglePageNav( $args ) {
		...
		?>
		<nav role="navigation" id="<?php echo esc_attr( $args['id'] ); ?>" class="<?php echo $args['class']; ?>">
	...
	function getThemeRexCalendar($onlyFirstLetter = true, $get_month = 0, $get_year = 0, $opt=array()) {
		...
		for ($i=0; $i<count($tmp); $i++) {
			...
			if ($tmp[$i]=='tribe_events') {
				if (!class_exists('TribeEvents')) continue;
				$events_present = true;
			} else {
		...
		if ( !$posts ) {
			$gotsome = $wpdb->get_var("SELECT 1 as test FROM $sql_from_full WHERE $sql_post_type_full AND $sql_status LIMIT 1");
			if ( !$gotsome ) {
				$cache[ $key ] = '';
	...
	function getPostData(&$opt, $post_obj=null) {
		...
		if (!empty($opt['date_format'])) {
			...
			if ($post_date_part1.$post_date_part2!='') {
				$post_date = prepareDateForTranslation($post_date_part1 . ($post_date_part2!='' ? ' '.$post_date_part2 : ''));
			}
		}
...
if (!function_exists('getPostsByMetaValue')) {
	function getPostsByMetaValue($meta_key, $meta_value, $return_format=OBJECT) {
		global $wpdb;
		$where = array();
		if ($meta_key) $where[] = 'meta.meta_key="' . $meta_key . '"';
		if ($meta_value) $where[] = 'meta.meta_value="' . $meta_value . '"';
